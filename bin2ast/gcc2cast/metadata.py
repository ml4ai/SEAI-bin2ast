...
class AutoMATESBaseEnum(Enum):
    ...
    @classmethod
    @abstractmethod
    def from_str(cls, data: str, child_cls: Type):
        try:
            # return getattr(cls, data.upper())
            return getattr(child_cls, data.upper())
        except AttributeError:
            raise MissingEnumError(
                # f"No matching value found in {cls.__name__} for {data}"
...
class MetadataType(AutoMATESBaseEnum):
    ...
    @classmethod
    def from_str(cls, data: str, child_type: None):  # (cls, data: str):
        return super().from_str(data, cls)  # (cls, data)

    @classmethod
    def get_metadata_class(cls, mtype: MetadataType) -> Type[TypedMetadata]:
